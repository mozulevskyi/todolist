...
class CommentsContainer extends Component {
  ...
  componentDidMount() {
    axios.get(`http://localhost:3001/projects/${this.props.project.id}/tasks/${this.props.task.id}/comments.json`)
      .then(response => {
  ...
  addNewComment = () => {
    axios.post(`http://localhost:3001/projects/${this.props.project.id}/tasks/${this.props.task.id}/comments`, {comment: {body: ''}})
      .then(response => {
  ...
  deleteComment = (id) => {
    axios.delete(`http://localhost:3001/projects/${this.props.project.id}/tasks/${this.props.task.id}/comments/${id}`)
      .then(response => {
